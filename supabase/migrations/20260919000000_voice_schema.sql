-- 목소리(C 담당) 스키마 — 0001_init.sql(A의 골격) 위에 얹는다.
--
-- 0001_init.sql이 accounts · subjects · consents · voice_profiles를 골격으로 만들었다. 여기서:
--   1) consents · voice_profiles에 제약을 더한다
--   2) phrase_audio(사전 합성 오디오)를 만든다
--   3) accounts · subjects · consents · voice_profiles의 "for all" 정책을 작업별 정책으로 바꾼다
--   4) private bucket voice-refs · phrase-audio를 만든다
--
...
--
-- 클라이언트(authenticated)에게는 막고 서버(service_role) 함수(src/services)만 하는 것:
--   - voice_profiles · phrase_audio 쓰기: 임의 오디오를 "그 사람 목소리"로 끼워 넣거나 동의 없이 프로필을 만드는 경로가 된다
--   - consents 철회(update) · subjects 삭제 · accounts 삭제: 목소리 파기(ElevenLabs voice, Storage 파일)를 동반해야 한다.
--     DB cascade는 Storage 객체와 외부 API의 voice를 지우지 못한다 (src/services/purge.ts)
--   - consents 삭제: 동의 기록은 철회(revoked_at)로만 남긴다 (감사 추적)
-- storage.objects에 voice-refs · phrase-audio 정책을 두지 않는다. 업로드·다운로드는 서버가 발급한 서명 URL로만 한다.

-- 1) 제약

alter table public.consents
  add constraint consents_kind_check check (kind in (
    'biometric', 'voice_self', 'voice_family', 'research_use',
    'overseas_transfer', 'research_video', 'voice_retention'
  )),
  add constraint consents_granted_by_check check (granted_by in ('self', 'legal_guardian', 'family')),
  add constraint consents_id_subject_key unique (id, subject_id);

alter table public.voice_profiles
  alter column provider_voice_id set not null,
  -- 프리셋은 넣지 않는다 (위 주석)
  add constraint voice_profiles_source_check check (source in ('family', 'self')),
  add constraint voice_profiles_id_subject_key unique (id, subject_id),
  -- 다른 대상자의 동의로 프로필을 만들 수 없다
  add constraint voice_profiles_consent_subject_fkey
...
drop policy "own account" on public.accounts;
drop policy "own subjects" on public.subjects;
drop policy "own consents" on public.consents;
drop policy "own voice_profiles" on public.voice_profiles;

-- delete 정책 없음 — 계정·대상자 삭제는 서버 함수가 목소리를 파기한 뒤에 한다
...
-- update·delete 정책 없음 — 철회는 서버 함수(revokeConsent)가 파기와 함께 한다, 기록은 지우지 않는다
...
-- 쓰기 정책 없음 — 쓰기는 서버(service_role)만 한다
...
-- 4) Storage — private bucket. storage.objects 정책은 만들지 않는다 (서명 URL 전용)
